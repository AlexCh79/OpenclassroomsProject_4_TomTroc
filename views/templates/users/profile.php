    ...
    <div class="desktop-profile">
        <table aria-label="Liste des livres de l'utilisateur" class="table-books">
            ...
            <tbody>
                <?php foreach ($books as $book) { ?>
                <tr>
                    <th scope="row"><a href="index.php?action=book&id=<?= $book->getId() ?>" aria-label="Vers les détails du livre"><img  aria-label="Couverture" class="user-book-cover" src="<?= urldecode($book->getImage()) ?>"></a></th>
